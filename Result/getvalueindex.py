...
if __name__ == "__main__":
	...
	vocabularys = model.vectorizer.vocabulary_
	dirc = {}
	count = 0
	for vocabulary in vocabularys:
		dirc[vocabularys[vocabulary]] = vocabulary
		#dirc[count] = vocabulary
		count = count + 1
	print type(vocabularys)
	voca = []
	for cate in cates:
		...
		for a in array:
			if a!=0:
				voca.append(dirc[count])
				count = count + 1
	setvocabulary = set(voca)
	file1 = open("tmp.txt","w")
	for voca in setvocabulary:
		file1.write(voca.encode('utf8')+"\n")
	file1.close()
